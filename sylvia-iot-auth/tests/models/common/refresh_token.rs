...
use sylvia_iot_auth::models::refresh_token::{QueryCond, RefreshToken, RefreshTokenModel};

/// Test `add()`.
pub fn add(runtime: &Runtime, model: &dyn RefreshTokenModel) -> Result<(), String> {
    let token = RefreshToken {
        refresh_token: "token_add_none".to_string(),
    ...
    let get_token = match runtime.block_on(async { model.get(&token.refresh_token).await }) {
    ...
    let token = RefreshToken {
        refresh_token: "token_add_some".to_string(),
        expires_at: Utc::now().trunc_subsecs(3),
        scope: Some("scope_add".to_string()),
        client_id: "client_id_add".to_string(),
        redirect_uri: "redirect_uri_add".to_string(),
...
pub fn add_dup(runtime: &Runtime, model: &dyn RefreshTokenModel) -> Result<(), String> {
    let token = RefreshToken {
...
pub fn del_by_token(runtime: &Runtime, model: &dyn RefreshTokenModel) -> Result<(), String> {
    ...
    if let Err(e) = runtime.block_on(async {
        ...
        token.refresh_token = token_not_del.to_string();
        model.add(&token).await?;
...
/// Test `del()` twice.
pub fn del_twice(runtime: &Runtime, model: &dyn RefreshTokenModel) -> Result<(), String> {
    let token_del = "token_del";
    let token = RefreshToken {
        refresh_token: token_del.to_string(),
        expires_at: Utc::now().trunc_subsecs(3),
...
pub fn del_by_client_id(runtime: &Runtime, model: &dyn RefreshTokenModel) -> Result<(), String> {
    let token_del1 = "token_del1";
    let token_del2 = "token_del2";
    let token_not_del = "token_not_del";
    let mut token = RefreshToken {
        refresh_token: token_del1.to_string(),
        expires_at: Utc::now().trunc_subsecs(3),
        scope: Some("scope_del".to_string()),
        client_id: "client_id_del".to_string(),
        redirect_uri: "redirect_uri_del".to_string(),
    ...
    if let Err(e) = runtime.block_on(async {
        model.add(&token).await?;
        token.refresh_token = token_del2.to_string();
        model.add(&token).await?;
        token.refresh_token = token_not_del.to_string();
...
pub fn del_by_user_id(runtime: &Runtime, model: &dyn RefreshTokenModel) -> Result<(), String> {
    let token_del1 = "token_del1";
    let token_del2 = "token_del2";
    let token_not_del = "token_not_del";
    let mut token = RefreshToken {
...
pub fn del_by_user_client(runtime: &Runtime, model: &dyn RefreshTokenModel) -> Result<(), String> {
    let token_del = "token_del";
    let token_not_del = "token_not_del";
    let mut token = RefreshToken {
        refresh_token: token_del.to_string(),

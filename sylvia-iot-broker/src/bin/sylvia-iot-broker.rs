...
async fn main() -> std::io::Result<()> {
    ...
    let broker_state = match routes::new_state("/broker", &conf.broker).await {
        Err(e) => {
    ...
    let mut serv = HttpServer::new(move || {
        App::new()
            ...
            .service(routes::new_service(&broker_state))
...
fn init_config() -> Result<AppConfig, Box<dyn StdError>> {
    ...
    if let Some(v) = args.get_one::<String>("file") {
        ...
        return Ok(AppConfig {
            ...
            broker: libs::config::apply_default(&conf.broker),
        });
    }
    ...
    Ok(AppConfig {
        ...
        broker: libs::config::read_args(&args),
    })
}

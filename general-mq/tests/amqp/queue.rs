...
use general_mq::{
    ...
    AmqpConnection, AmqpConnectionOptions, AmqpQueue, AmqpQueueOptions, Queue as MqQueue,
    QueueOptions as MqQueueOptions,
};
...
#[derive(Default)]
struct Resources {
    pub conn: Vec<Box<AmqpConnection>>,
    pub queues: Vec<Box<AmqpQueue>>,
...
pub fn new_default(_context: &mut SpecContext<TestState>) -> Result<(), String> {
    let conn = match AmqpConnection::new(AmqpConnectionOptions::default()) {
        Err(e) => return Err(format!("AmqpConnection::new() error: {}", e)),
        Ok(conn) => conn,
    };
    ...
    let queue = AmqpQueue::new(opts, &conn);
...
pub fn properties(_context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let queue = match AmqpQueue::new(opts, &conn) {
        Err(e) => return Err(format!("AmqpQueue::new() error: {}", e)),
        Ok(q) => q,
    };
...
pub fn connect_no_handler(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let mut conn = match AmqpConnection::new(AmqpConnectionOptions::default()) {
        Err(e) => return Err(format!("AmqpConnection::new() error: {}", e)),
        Ok(conn) => conn,
    };
    ...
    let mut queue = match AmqpQueue::new(opts, &conn) {
        Err(e) => return Err(format!("AmqpQueue::new() error: {}", e)),
        Ok(q) => q,
    };
...
pub fn send_error(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let queue = match AmqpQueue::new(opts, &conn) {
        Err(e) => return Err(format!("AmqpQueue::new() recv error: {}", e)),
        Ok(q) => q,
    };
    ...
    let queue = match AmqpQueue::new(opts, &conn) {
        Err(e) => return Err(format!("AmqpQueue::new() send error: {}", e)),
        Ok(conn) => conn,
    };
    match state.runtime.block_on(queue.send_msg(vec![])) {
        Err(_) => (),
...
pub fn data_nack(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    state.runtime.block_on(async move {
        {
            *handler.use_nack.lock().unwrap() = true;
        }
        let queue = match resources.queues.get_mut(0) {
            None => return Err(format!("should have send queue")),
            Some(q) => q,
        ...
        while retry > 0 {
            ...
            let len;
            {
                len = handler.nack_messages.lock().unwrap().len();
            }
            if len > 0 {
                break;
            }
        ...
        if retry == 0 {
            return Err(format!("cannot receive 1 for nack"));
        }
        {
            *handler.use_nack.lock().unwrap() = false;
        }
        let mut retry = 150;
        while retry > 0 {
            time::sleep(Duration::from_millis(10)).await;
...
fn create_msg_rsc(
    ...
    let queue = match AmqpQueue::new(send_opts, &conn) {
        Err(e) => return Err(format!("AmqpQueue::new() send error: {}", e)),
        Ok(q) => q,
    };
    state.queues.push(Box::new(queue.clone()));
    ...
    for _ in 0..receiver_count {
        ...
        let mut queue = match AmqpQueue::new(recv_opts, &conn) {
            Err(e) => return Err(format!("AmqpQueue::new() recv error: {}", e)),
            Ok(q) => q,
        };
        state.queues.push(Box::new(queue.clone()));

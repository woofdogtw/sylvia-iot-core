...
pub fn reg_args(cmd: Command) -> Command {
    cmd.about("Network route management")
        .subcommand(
            Command::new("add")
                .about("Add network route")
                .arg(
                    Arg::new("netid")
                        .short('n')
                        .long("netid")
        ...
        .subcommand(
            Command::new("delete").about("Delete a network route").arg(
                Arg::new("routeid")
                    .short('i')
                    .long("routeid")
                    .help("Route ID")
...
async fn add(config: &Config, args: &ArgMatches) -> Result<PostResData, ErrResp> {
    ...
    let uri = format!("{}/api/v1/network-route", config.coremgr);
...
async fn count(config: &Config, args: &ArgMatches) -> Result<GetCountResData, ErrResp> {
    ...
    let uri = match serde_urlencoded::to_string(&query) {
        ...
        Ok(query) => format!("{}/api/v1/network-route/count?{}", config.coremgr, query),
    };
    let mut token = storage.access_token;
...
async fn list(config: &Config, args: &ArgMatches) -> Result<Vec<GetResData>, ErrResp> {
    ...
    let uri = match serde_urlencoded::to_string(&query) {
        ...
        Ok(query) => format!("{}/api/v1/network-route/list?{}", config.coremgr, query),
    };
    let mut token = storage.access_token;

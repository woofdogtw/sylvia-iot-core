...
use super::{config, get_csv_filename, refresh, validate_timestr, Config, API_RETRY};
...
pub fn reg_args(cmd: Command) -> Command {
    cmd.about("Application uplink data")
        .subcommand(
            Command::new("count")
...
async fn count(config: &Config, args: &ArgMatches) -> Result<GetCountResData, ErrResp> {
    ...
    let query = GetCountReq {
        user: match args.get_one::<String>("user") {
            None => None,
    ...
    let uri = match serde_urlencoded::to_string(&query) {
        ...
        Ok(query) => format!("{}/api/v1/coremgr-opdata/count?{}", config.data, query),
    };
...
async fn list(config: &Config, args: &ArgMatches) -> Result<Vec<GetResData>, ErrResp> {
    ...
    let uri = match serde_urlencoded::to_string(&query) {
        ...
        Ok(query) => format!("{}/api/v1/coremgr-opdata/list?{}", config.data, query),
    };
    let mut token = storage.access_token;

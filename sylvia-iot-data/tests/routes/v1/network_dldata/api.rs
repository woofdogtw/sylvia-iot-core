...
pub fn get_count_invalid_token(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let req = TestRequest::get().uri("/data/api/v1/network-dldata/count");
...
pub fn get_list_invalid_token(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let req = TestRequest::get().uri("/data/api/v1/network-dldata/list");
    test_invalid_token(runtime, &routes_state, req)
}
...
fn test_get_count(
    ...
    let uri = match param {
        None => "/data/api/v1/network-dldata/count".to_string(),
        Some(param) => format!(
            "/data/api/v1/network-dldata/count?{}",
...
fn test_get_list(
    ...
    let uri = match param {
        None => "/data/api/v1/network-dldata/list".to_string(),
        Some(param) => format!(
            "/data/api/v1/network-dldata/list?{}",
            serde_urlencoded::to_string(&param).unwrap()
        ),
    };
    let req = TestRequest::get()
...
fn test_get_list_format_csv(
    ...
    let fields =
        b"dataId,proc,pub,resp,status,unitId,deviceId,networkCode,networkAddr,data,extension";
    let mut count = 0;

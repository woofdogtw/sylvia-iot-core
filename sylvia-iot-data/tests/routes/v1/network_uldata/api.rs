...
pub fn get_count_not_exist_unit(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let uri = "/data/api/v1/network-uldata/count";
...
pub fn get_count_invalid_token(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let req = TestRequest::get().uri("/data/api/v1/network-uldata/count");
...
pub fn get_list_not_exist_unit(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let uri = "/data/api/v1/network-uldata/list";
    let code = "err_data_unit_not_exist";
...
pub fn get_list_invalid_param(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    query.insert(
        "sort".to_string(),
        Value::String("proc:asc,time:true".to_string()),
    );
    test_get_400(runtime, &routes_state, TOKEN_MANAGER, uri, &query, code)?;
...
pub fn get_list_invalid_token(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let req = TestRequest::get().uri("/data/api/v1/network-uldata/list");
    test_invalid_token(runtime, &routes_state, req)
}
...
fn test_get_count(
    ...
    let uri = match param {
        None => "/data/api/v1/network-uldata/count".to_string(),
        Some(param) => format!(
            "/data/api/v1/network-uldata/count?{}",
...
fn test_get_list(
    ...
    let uri = match param {
        None => "/data/api/v1/network-uldata/list".to_string(),
        Some(param) => format!(
            "/data/api/v1/network-uldata/list?{}",
            serde_urlencoded::to_string(&param).unwrap()
        ),
    };
    let req = TestRequest::get()
    ...
    for info in body.data.iter() {
        ...
        if let Err(_) = DateTime::parse_from_rfc3339(info.time.as_str()) {
    ...
    match runtime.block_on(async {
        let data = NetworkUlData {
            ...
            unit_code: match use_some {
                false => None,
                true => Some("unit_code".to_string()),
            },
            network_code: "network_code".to_string(),
            network_addr: "network_addr".to_string(),
            unit_id: match use_some {
                false => None,
                true => Some(UNIT_OWNER.to_string()),
            },

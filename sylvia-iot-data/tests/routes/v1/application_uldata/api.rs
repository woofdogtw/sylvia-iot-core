...
use sylvia_iot_data::{models::application_uldata::ApplicationUlData, routes};
...
pub fn get_count(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let mut param = request::GetCount {
        ...
        tstart: Some(now.timestamp_millis() + 8),
        ..Default::default()
    };
    test_get_count(runtime, routes_state, TOKEN_MANAGER, Some(&param), 2)?;
    test_get_count(runtime, routes_state, TOKEN_OWNER, Some(&param), 2)?;
...
pub fn get_count_not_exist_unit(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let uri = "/data/api/v1/application-uldata/count";
...
pub fn get_count_invalid_token(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let req = TestRequest::get().uri("/data/api/v1/application-uldata/count");
...
pub fn get_list_sort(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    param.sort_vec = Some(vec![("time", true)]);
    test_get_list_sort(
        runtime,
        &routes_state,
        TOKEN_MANAGER,
    ...
    param.sort_vec = Some(vec![("time", false)]);
    test_get_list_sort(
        runtime,
        &routes_state,
        TOKEN_MANAGER,
...
pub fn get_list_not_exist_unit(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let uri = "/data/api/v1/application-uldata/list";
    let code = "err_data_unit_not_exist";
...
pub fn get_list_invalid_param(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    query.insert(
        "sort".to_string(),
        Value::String("proc:asc,pub:true".to_string()),
    );
    test_get_400(runtime, &routes_state, TOKEN_MANAGER, uri, &query, code)?;
...
pub fn get_list_invalid_token(context: &mut SpecContext<TestState>) -> Result<(), String> {
    ...
    let req = TestRequest::get().uri("/data/api/v1/application-uldata/list");
    test_invalid_token(runtime, &routes_state, req)
}
...
fn test_get_count(
    ...
    let uri = match param {
        None => "/data/api/v1/application-uldata/count".to_string(),
        Some(param) => format!(
            "/data/api/v1/application-uldata/count?{}",
...
fn test_get_list(
    ...
    let uri = match param {
        None => "/data/api/v1/application-uldata/list".to_string(),
        Some(param) => format!(
            "/data/api/v1/application-uldata/list?{}",
            serde_urlencoded::to_string(&param).unwrap()
        ),
    };
    let req = TestRequest::get()
    ...
    for info in body.data.iter() {
        ...
        if let Err(_) = DateTime::parse_from_rfc3339(info.publish.as_str()) {
            return Err(format!("pub {} format error", info.publish.as_str()));
        } else if let Err(_) = DateTime::parse_from_rfc3339(info.time.as_str()) {
            return Err(format!("time {} format error", info.time.as_str()));
        }
        if proc > prev_proc {
            return Err(format!(
                "proc order error: {} - {}",
...
fn test_get_list_format_csv(
    ...
    let fields =
        b"dataId,proc,pub,unitCode,networkCode,networkAddr,unitId,deviceId,time,profile,data,extension";
    let mut count = 0;
...
fn count_list_dataset(
    ...
    let mut data = ApplicationUlData {
        data_id: "data_id1".to_string(),
        proc: now,
        publish: now + Duration::milliseconds(6),
        unit_code: Some(UNIT_OWNER.to_string()),
    ...
    if let Err(e) = runtime.block_on(async {
        state.model.application_uldata().add(&data).await?;
...
fn add_offset_limit_data(
    ...
    match runtime.block_on(async {
        let data = ApplicationUlData {
            ...
            proc,
            publish: proc,
            unit_code: match use_some {
                false => None,
                true => Some("unit_id".to_string()),
            },

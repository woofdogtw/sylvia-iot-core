...
use sylvia_iot_data::models::application_uldata::{
    ApplicationUlData, ApplicationUlDataModel, ListOptions, ListQueryCond, QueryCond, SortCond,
    SortKey,
};
...
pub fn add(
    runtime: &Runtime,
    model: &dyn ApplicationUlDataModel,
    db: &dyn Db,
) -> Result<(), String> {
    ...
    let data = ApplicationUlData {
        ...
        proc: now,
        publish: now + TimeDelta::try_milliseconds(1).unwrap(),
...
pub fn add_dup(runtime: &Runtime, model: &dyn ApplicationUlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let data = ApplicationUlData {
        data_id: strings::random_id(&now, 8),
        proc: now,
...
/// Test `del()` twice.
pub fn del_twice(runtime: &Runtime, model: &dyn ApplicationUlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let data = ApplicationUlData {
        data_id: "data_id1_1".to_string(),
        proc: now,
...
/// Test `count()`.
pub fn count(runtime: &Runtime, model: &dyn ApplicationUlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let mut data = ApplicationUlData {
        data_id: "data_id1".to_string(),
        proc: now,
    ...
    let cond = ListQueryCond {
        pub_gte: Some(now + TimeDelta::try_milliseconds(5).unwrap()),
...
/// Test `list()`.
pub fn list(runtime: &Runtime, model: &dyn ApplicationUlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let mut data = ApplicationUlData {
        data_id: "data_id1".to_string(),
        proc: now,
...
pub fn list_sort(runtime: &Runtime, model: &dyn ApplicationUlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let mut data = ApplicationUlData {
        data_id: "data_id1".to_string(),
        proc: now,
    ...
    opts.sort = Some(sort_cond.as_slice());
    let list = match runtime.block_on(async { model.list(&opts, None).await }) {
        Err(e) => return Err(format!("list pub-asc result error: {}", e)),
        Ok((list, _)) => list,
    };
    expect(list.len()).to_equal(5)?;
    expect(list[0].data_id.as_str()).to_equal("data_id1")?;
    ...
    opts.sort = Some(sort_cond.as_slice());
    let list = match runtime.block_on(async { model.list(&opts, None).await }) {
        Err(e) => return Err(format!("list pub-desc result error: {}", e)),
        Ok((list, _)) => list,
    };
    expect(list.len()).to_equal(5)?;
    expect(list[0].data_id.as_str()).to_equal("data_id5")?;
...
pub fn list_cursor(runtime: &Runtime, model: &dyn ApplicationUlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let mut data = ApplicationUlData {
        data_id: "data_id1".to_string(),
        proc: now,

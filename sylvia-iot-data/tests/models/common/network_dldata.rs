...
use sylvia_iot_data::models::network_dldata::{
    ListOptions, ListQueryCond, NetworkDlData, NetworkDlDataModel, QueryCond, SortCond, SortKey,
...
/// Test `add()`.
pub fn add(runtime: &Runtime, model: &dyn NetworkDlDataModel, db: &dyn Db) -> Result<(), String> {
    ...
    let data = NetworkDlData {
        ...
        proc: now,
        publish: now + TimeDelta::try_milliseconds(2).unwrap(),
        resp: None,
        status: 1,
...
pub fn add_dup(runtime: &Runtime, model: &dyn NetworkDlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let data = NetworkDlData {
        data_id: strings::random_id(&now, 8),
        proc: now,
...
/// Test `del()` twice.
pub fn del_twice(runtime: &Runtime, model: &dyn NetworkDlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let data = NetworkDlData {
        data_id: "data_id1_1".to_string(),
        proc: now,
...
pub fn update_not_exist(runtime: &Runtime, model: &dyn NetworkDlDataModel) -> Result<(), String> {
...
/// Test `count()`.
pub fn count(runtime: &Runtime, model: &dyn NetworkDlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let mut data = NetworkDlData {
        data_id: "data_id1".to_string(),
        proc: now,
    ...
    let cond = ListQueryCond {
        pub_gte: Some(now + TimeDelta::try_milliseconds(6).unwrap()),
        pub_lte: Some(now + TimeDelta::try_milliseconds(9).unwrap()),
        ..Default::default()
    };
    let count = match runtime.block_on(async { model.count(&cond).await }) {
        Err(e) => return Err(format!("count pub result error: {}", e)),
        Ok(count) => count,
    };
    expect(count).to_equal(4)?;
...
/// Test `list()`.
pub fn list(runtime: &Runtime, model: &dyn NetworkDlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let mut data = NetworkDlData {
        data_id: "data_id1".to_string(),
        proc: now,
    ...
    opts.cond = &cond;
    let list = match runtime.block_on(async { model.list(&opts, None).await }) {
        Err(e) => return Err(format!("list pub result error: {}", e)),
        Ok((list, _)) => list,
    };
    expect(list.len()).to_equal(4)?;
...
pub fn list_sort(runtime: &Runtime, model: &dyn NetworkDlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let mut data = NetworkDlData {
        data_id: "data_id1".to_string(),
        proc: now,
...
pub fn list_offset_limit(runtime: &Runtime, model: &dyn NetworkDlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let mut data = NetworkDlData {
        data_id: "data_id1".to_string(),
        proc: now,
...
pub fn list_cursor(runtime: &Runtime, model: &dyn NetworkDlDataModel) -> Result<(), String> {
    let now = Utc::now().trunc_subsecs(3);
    let mut data = NetworkDlData {
        data_id: "data_id1".to_string(),
        proc: now,

...
async fn main() -> std::io::Result<()> {
    ...
    let coremgr_state = match routes::new_state("/coremgr", &conf.coremgr).await {
        Err(e) => {
...
fn init_config() -> Result<AppConfig, Box<dyn StdError>> {
    ...
    if let Some(v) = args.get_one::<String>("file") {
        ...
        return Ok(AppConfig {
            ...
            coremgr: libs::config::apply_default(&conf.coremgr),
        });
    }
    ...
    Ok(AppConfig {
        ...
        coremgr: libs::config::read_args(&args),
    })
}
